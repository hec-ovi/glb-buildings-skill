...
export type FlagSpec = Record<string, { type: 'string' }>;

export type Parsed = { positionals: string[]; values: Record<string, string | undefined> };
...
export function parse(args: string[], options: FlagSpec): Parsed {
  try {
    ...
    return { positionals: parsed.positionals, values: parsed.values as Record<string, string | undefined> };
  } catch (error) {
...
export function metres(value: string | undefined, what: string): number | undefined {
...
export function count(value: string | undefined, what: string): number | undefined {
  if (value === undefined) return undefined;
  const parsed = Number(value);
...
export function degrees(value: string | undefined): number | undefined {
  if (value === undefined) return undefined;
  const parsed = Number(value);
